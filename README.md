...
Claude가 코드를 작성하고, Gemini가 리뷰와 리서치를 담당하는 AI 기반 개발 워크스페이스.  
...
> 좌측: Claude Code 터미널 (대기 중) / 우측 상단: Gemini 패널 (변경사항 리뷰 버튼) / 우측 하단: Dev Server 멀티탭 / 하단: 이벤트 콘솔

### 공유 브라우저 + Gemini 리뷰
![브라우저](docs/browser.png)
> Claude가 코드 분석 중. 우측에 Chrome 공유 브라우저(CDP :9222) 실행, Gemini 리뷰 완료. Dev Server 탭 두 개 동시 실행.
...
| Gemini CLI | 리뷰/리서치 | `gemini --version` |
...
| Gemini로 전송 | 선택 영역을 Gemini 입력창에 자동 삽입 |
...
**응답 스트리밍**  
Gemini CLI의 stdout을 라인 단위로 읽어 실시간으로 패널에 표시한다.  
응답이 완료되면 로딩 인디케이터가 사라지고 카드가 확정된다.

각 요청은 접기/펼치기 가능한 카드로 쌓여 히스토리처럼 탐색할 수 있다.

**변경사항 리뷰 버튼:**  
Claude가 코드를 수정한 후 **변경사항 리뷰** 클릭 → `git diff HEAD` 자동 실행 → diff가 Gemini 입력창에 채워짐 → 전송하면 리뷰 시작.  
...
13:45:10  INFO  Gemini review 요청: 로그인 로직 변경사항…
13:45:18  OK    Gemini review 완료
...
Kuro/
├── electron/
│   ├── main.ts          # IPC 핸들러 (PTY, MCP, Gemini, 브라우저, git, HTTP 서버)
...
│   │   │   ├── GeminiPanel.tsx       # 리뷰/리서치 UI + 스트리밍 + git diff 리뷰
...
└── scripts/
    ├── ask-gemini.ps1               # Gemini CLI 래퍼 PowerShell 스크립트
...
Main Process (main.ts)
    ...
    ├─ child_process   → Gemini PowerShell 스크립트 / git diff / tasklist
...
```
[Renderer] window.kuro.geminiInvoke({ mode, input })
    ↓
[Main] child_process.spawn('powershell.exe', ['-File', 'ask-gemini.ps1'])
    │  stdin ← JSON.stringify({ mode, input, context })
    │
    │  stdout (스트리밍)
    ├─→ webContents.send('gemini:stream', chunk)  ← 실시간 표시
    │
...
`scripts/ask-gemini.ps1`의 마지막 줄이 Gemini CLI를 호출한다.
...
Gemini CLI 버전에 따라 플래그가 다를 수 있다. `gemini --help`로 확인 후 조정한다.
...
| AI - 리뷰/리서치 | Gemini CLI (child_process) |
